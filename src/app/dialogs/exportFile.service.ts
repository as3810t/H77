	...
	exportFile(fileName, type, data, keyboard, settings) {
		return new Promise(function(resolve, reject) {
			...
			for(var i = 0; i < keyboard.length; i++) {
				result[window.XLSX.utils.encode_col(1+0*keyboard.length+i) + '1'] = {t: 's', v: 'Latencia'};
				result[window.XLSX.utils.encode_col(1+1*keyboard.length+i) + '1'] = {t: 's', v: 'Frekvencia'};
				result[window.XLSX.utils.encode_col(1+2*keyboard.length+i) + '1'] = {t: 's', v: 'Idő'};
				result[window.XLSX.utils.encode_col(1+3*keyboard.length+i) + '1'] = {t: 's', v: 'Időszázalék'};
			...
			if(type == 'csv') {
				...
				wb.SheetNames.push('Eseménylista');
				var eventList = {
					A1: {t: 's', v: '#'},
					B1: {t: 's', v: 'Billentyű'},
					C1: {t: 's', v: 'Név'},
				...
				wb.Sheets['Eseménylista'] = eventList;

				wb.SheetNames.push('Események');
				var board = {
					A1: {t: 's', v: 'Billentyű'},
					B1: {t: 's', v: 'Név'},
					C1: {t: 's', v: 'Típus'}
